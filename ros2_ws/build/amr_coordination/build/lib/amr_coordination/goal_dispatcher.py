...
class GoalDispatcher(Node):
    ...
    def __init__(self):
        ...
        self._robot_state: dict[str, dict] = {
            rid: {'status': RobotStatus.IDLE, 'pose_x': 0.0, 'pose_y': 0.0}
    ...
    def _status_cb(self, msg: RobotStatus, robot_id: str):
        ...
        self._robot_state[robot_id].update({
            'status': msg.status,
            'pose_x': msg.pose_x,
            'pose_y': msg.pose_y,
        })
    ...
    def _try_dispatch(self, point: PointStamped):
        ...
        for rid, state in self._robot_state.items():
            ...
            dx = point.point.x - state['pose_x']
            dy = point.point.y - state['pose_y']
        ...
        if best_robot is None:
            self._log(
                ...
                f'({point.point.x:.2f}, {point.point.y:.2f}). '
                f'Queue depth: {len(self._goal_queue) + 1}'
            )
            self._goal_queue.append(point)
            self._publish_queue_size()
            return

        self._assign_goal(best_robot, point, best_dist)
    ...
    def _drain_queue(self):
        ...
        while self._goal_queue:
            ...
            if not has_idle:
                break
            point = self._goal_queue.popleft()
            self._publish_queue_size()
            self._try_dispatch(point)

    def _assign_goal(self, robot_id: str, point: PointStamped, distance: float):
        ...
        # Default orientation: face forward (w=1, z=0)
        goal_msg.pose.pose.orientation.w = self._default_orientation_w
        goal_msg.pose.pose.orientation.z = 0.0

        self._log(
            f'Assigning goal ({point.point.x:.2f}, {point.point.y:.2f}) '
        ...
        if not client.server_is_ready():
            ...
            self._goal_queue.appendleft(point)
            self._publish_queue_size()
            return

        send_future = client.send_goal_async(goal_msg)
        send_future.add_done_callback(
            lambda fut, rid=robot_id, pt=point: self._goal_response_cb(fut, rid, pt)
        )

    def _goal_response_cb(self, future, robot_id: str, point: PointStamped):
